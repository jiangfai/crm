...
function changePost(depId){
	//直接使用jquery的调用方式
	$.ajax({
		"url" : "${pageContext.request.contextPath}/ajaxByDepid.do",
		"data" : "depId="+depId,
		"type" : "post",
		"success" : function(data) {
			var innerhtml= "<option value='init'>--请选择--</option>";
			for (var i = 0; i < data.length; i++) {
				innerhtml=innerhtml+"<option value='"+data[i].postId+"'>"+data[i].postName+"</option>";
			}
			document.getElementById("postId").innerHTML=innerhtml;
		},
		"error" : function() {
			alert("请求失败");
		}
...
<form action="${pageContext.request.contextPath}/addStaff.do" method="post">
